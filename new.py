...
def index():
    if request.method == 'POST':
        ...
        a = multiprocessing.Process(target=inference_process, args=[data,npimg])
        a.name = "main_t"
        fifo_queue.put(a)
        a.start()
        return "200"
...
def inference_process(data,npimg):
    # w = Watcher(0)
    #t = TimeLimit(0)
    od = eval(data)
    #print(od,type(od))
    ...
    if count >= 1:
        try:
            ...
            #tme = t.check_value(duration)
            process_list=[Process.name for Process in multiprocessing.enumerate()]
            print(process_list,"list")
            #if "running" in process_list:
            #print("t",t.check_value())
            # if len(process_list)>=3 and "main_t" in process_list:
            #     print(process_list,"pass")
            #     pass
                

            # else:
            #if t.check_value() > duration:
            cc = multiprocessing.Process(target = switch_asset,args = [asset,device_data[0],duration])
            cc.name = "running"
            fifo_queue.put(cc)
            cc.start()
            print(process_list,"run")
            #switch_asset(asset,device_data[0])
            
            #t = TimeLimit(int(time.time()))
            #w.check_value(count)
            print(process_list,"done")
